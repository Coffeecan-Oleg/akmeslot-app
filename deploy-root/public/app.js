...
class AkmeSlotApp {
  ...
  async init() {
    ...
    const authId = bxParams.AUTH_ID;
    const domain = bxParams.DOMAIN;
    const userId = bxParams.USER_ID;
    
    console.log("[AkmeSlot] AUTH_ID:", authId ? "present" : "missing");
    console.log("[AkmeSlot] DOMAIN:", domain);
    ...
    try {
      await this.loadUser(authId, domain, userId);
  ...
  async loadUser(authId, domain, userId) {
    const apiUrl = `https://${domain}/rest/user.current?auth=${authId}`;
